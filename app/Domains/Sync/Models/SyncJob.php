...
class SyncJob extends BaseModel
{
    ...
    {
        return $q->where('status', SyncStatus::Pending->value);
    }

    public function scopeRunning($q)
    {
        return $q->where('status', SyncStatus::Running->value);
    }

    public function scopeFailed($q)
    {
        return $q->where('status', SyncStatus::Failed->value);
    }
}
